	...
	//$(window).bind("load resize", function(){
		if ($("nav").width() > 100) {
			$("nav ul li:has(ul)").click(function(){
			    if(!$(this).find("ul.submenu").hasClass("MenuShowed")){
			        $(this).find("ul.submenu").addClass("MenuShowed");
				    $(this).find("ul.submenu").stop("true", "true").slideDown(500);
				}
				else
				{
				    $(this).find("ul.submenu").removeClass("MenuShowed");
...
function onPopupVirtualBetting(bettype)
{
    window.open("/TotoBettingMng/popVirtualBetting.aspx?bettype=" + bettype, 'VirtualBetting', 'left=100,top=50,width=1300,height=700,resizable=no,scrollbars=yes');
}

function onPopupSendMsg(strID)
{    
    if(typeof(strID) == "undefined") {
        strID = "";
    }
    window.open("/popSendMsg.aspx?sendid=" + strID, 'SendMsg', 'left=400,top=200,width=540,height=440,resizable=no,scrollbars=yes');
}
function onPopupSendSms(strID)
{    
    if(typeof(strID) == "undefined") {
        strID = "";
    }
    window.open("/popSendSms.aspx?sendid=" + strID, 'SendSms', 'left=400,top=200,width=540,height=400,resizable=no,scrollbars=yes');
}
function onPopupBoardGameList(uID)
{
    if(typeof(uID) == "undefined")
        uID = "";
    window.open("/popBoardGameBetList.aspx?mid=" + uID, 'BoardGameBetList', 'left=200,top=100,width=800,height=600,resizable=no,scrollbars=yes');
}
function onPopupMemberInfo(uID)
{
    if(typeof(uID) == "undefined")
        uID = "";
    window.open("/popMemberInfo.aspx?uid=" + uID, 'MemberInfo', 'left=200,top=100,width=1200,height=600,resizable=no,scrollbars=yes');
}